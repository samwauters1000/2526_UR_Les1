...
function InternalPDFViewer({ file }: { file: string }) {
  ...
  if (!libraries)
    return <div className="text-foreground/20 text-sm italic">Initializing...</div>;
  ...
  return (
    <div className="flex flex-col items-center w-full bg-transparent">
      <div
        ref={containerRef}
        className="w-full flex justify-center overflow-hidden rounded-sm shadow-xl bg-white min-h-[300px]"
      >
        <Document
          file={file}
          ...
          loading={
            <div className="py-20 text-black/20 italic">Loading page...</div>
          }
      ...
      {renderNav && numPages > 1 && (
        <div className="flex items-center gap-6 mt-4 bg-black/20 px-4 py-1 rounded-full backdrop-blur-sm">
          ...
          <span className="text-[10px] tracking-widest text-foreground/40 uppercase font-bold">
            {pageNumber} / {numPages}
          </span>
          <button
            type="button"
...
const PDFViewer = dynamic(() => Promise.resolve(InternalPDFViewer), {
  ssr: false,
  loading: () => (
    <div className="text-foreground/20 text-sm italic font-light tracking-widest">
      Loading PDF...
    </div>
  ),
});
...
const projects = [
  {
    ...
    tag: "School Project",
    description:
      "Voor dit schoolproject ontwierp ik een volledig magazine van A tot Z. Van de lay-out en typografie tot de beeldkeuze en kleurpalet — alles werd zorgvuldig samengesteld om een coherent en professioneel geheel te vormen.",
  },
  {
    ...
    tag: "Graphic Design",
    description:
      "Een affiche ontworpen voor een theaterproductie. Het doel was om de sfeer en emotie van het stuk te vangen in één krachtig beeld. Ik experimenteerde met typografie, contrast en compositie.",
  },
  {
    ...
    description:
      "Een infographic over de gedragsregels in het openbaar vervoer van De Lijn. Het uitdagende was het omzetten van droge regelgeving naar een visueel aantrekkelijk verhaal via illustraties en heldere iconen.",
  },
  {
    title: "In the works...",
    type: "image",
    ...
    tag: "Studio Lux",
    description:
      "Dit project is momenteel in ontwikkeling. Meer details volgen binnenkort. Stay tuned voor updates over dit nieuwe werk.",
  },
];
    ...
    <main className="min-h-screen text-foreground bg-background">
      ...
      <div className="w-full px-4 max-w-sm mx-auto md:max-w-4xl md:px-6 pt-16 pb-24 flex flex-col gap-16 md:gap-24">
        ...
        <header
          ...
          className="opacity-0 translate-y-[20px] mb-4"
        >
        ...
        {projects.map((project, i) => (
          <article
            ...
            className={`flex flex-col md:flex-row gap-6 md:gap-12 items-center opacity-0 translate-y-[20px] ${
          ...
          >
            ...
            <div className="w-full md:w-1/2 flex flex-col items-center">
              <div className="w-full relative group">
                {project.type === "pdf" ? (
                  ...
                  <div
                    className="overflow-hidden rounded-sm shadow-xl w-full aspect-square bg-foreground/5 md:cursor-zoom-in"
                    onClick={() => setOverlay(project.file)}
                  >
                    <img
                      src={project.file}
                      className="w-full h-full grayscale hover:grayscale-0 transition-all duration-700 hover:scale-105 object-cover"
                      alt={project.title}
                    />
                  </div>
                )}
              ...
              <button
                onClick={() => setOverlay(project.file)}
                className="mt-4 flex items-center gap-4 group/btn"
              >
            ...
            <div className="w-full md:w-1/2">
              <span className="inline-block text-[10px] tracking-[0.25em] uppercase text-accent font-bold bg-accent/15 border border-accent/30 rounded-full px-4 py-1.5 mb-3">
                {project.tag}
              </span>
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-3 text-primary">
                {project.title}
              </h2>
              <p className="text-foreground/60 leading-relaxed font-light text-sm md:text-base">
                {project.description}
              </p>
            </div>
          </article>
