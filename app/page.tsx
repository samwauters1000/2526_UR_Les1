...
/* ─────────────────────────────────────────
   HERO  — fills viewport minus ~120px so
   the carousel top is always visible
───────────────────────────────────────── */
function Hero() {
  return (
    <section
    ...
    >
      {/* Purple glow blobs */}
      <div className="pointer-events-none absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full bg-[#7217E8] opacity-[0.07] blur-[80px]" />
      <div className="pointer-events-none absolute bottom-0 left-40 w-[300px] h-[300px] rounded-full bg-[#7217E8] opacity-[0.04] blur-[60px]" />

      <div className="animate-fadeIn relative z-10 flex flex-col items-center max-w-3xl">
        {/* Tag */}
        ...
        <h1
          ...
          style={{ fontSize: "clamp(3rem, 7vw, 5.5rem)", fontFamily: "var(--font-geist-sans)" }}
        >
          Hi, I&apos;m Sam
          <br />
        ...
        {/* Sub */}
        <p className="text-foreground/50 font-light text-lg leading-relaxed max-w-md mb-10">
        ...
        {/* Scroll hint */}
        <div className="mt-12 flex items-center gap-3 text-foreground/30 text-xs uppercase tracking-widest">
...
/* ─────────────────────────────────────────
   PROJECTS CAROUSEL
   Layout: far-left | near-left | CENTER | near-right | far-right
───────────────────────────────────────── */
const PROJECTS = [
...
// Distance from center: 0 = active, 1 = near side, 2 = far side
      ...
      <div className="flex flex-col items-center gap-3 mt-10">
        ...
        <div className="w-32 h-px bg-foreground/10 rounded-full overflow-hidden">
          <div
            ...
            style={{
              animation: paused ? "none" : "progressBar 4s linear forwards",
            }}
          />
        </div>
      </div>
...
const EXPERIENCE = [
  ...
  },
  {
    ...
    company: "Forum da Vinci",
    period: "2020 – 2023",
    desc: "Mijn into in de grafische wereld begon bij Forum da Vinci, waar ik de basis van design leerde en mijn passie voor visuele communicatie ontdekte.",
  },
]
    ...
    >
      ...
      <div className="w-full max-w-3xl flex flex-col">
        {EXPERIENCE.map((e, i) => (
          <div
            key={i}
            className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-4 md:gap-12 py-8 text-center md:text-left"
            style={{
              opacity: visible ? 1 : 0,
          ...
          >
            <div>
              <p className="text-foreground/40 text-sm">{e.period}</p>
              <p className="text-foreground/60 text-sm mt-1">{e.company}</p>
            </div>
            <div>
...
function Footer() {
  return (
    <footer className="px-8 py-10 flex flex-col md:flex-row items-center justify-center gap-6 md:gap-16">
      <p className="text-foreground/30 text-sm">
        © {new Date().getFullYear()} Sam Wauters. All rights reserved.
      </p>
